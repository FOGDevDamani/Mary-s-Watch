				...
				<div class="col-lg-8 col-sm-4">
						...
						Payments: <span v-for="item in monthItems" :key="item.name">
							...
							>
								<button
									...
									v-tippy="{ content: `${createdOnItem.createdOn}`, arrow: true, theme: 'translucent'}"
								>
									{{ item.name }}
								</button>
								<!-- <b-popover
					...
					<div>
						...
						<span v-for="ticket in project.checklistItems" :key="ticket" >
							<MWRouterLink :route="`/project-management`" class="inline-block" v-tippy="{ content: `${ticket.address}`, arrow: true}">
								{{ticket.createdOn.toDate().toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}}
							</MWRouterLink>
						</span>
					</span>
					</div>
...
const storeOwnerNoteTicket = useStoreOwnerNoteTicket()
...
const { tickets: noteTicket } = storeToRefs(storeOwnerNoteTicket)
