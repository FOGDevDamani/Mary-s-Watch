...
describe("createUser", () => {
  ...
  test("makes a POST request to create a new user", async () => {
    ...
    const newUser = await storeUser.addUserOnRegister(userData);
		...
		storeUser.addUserOnRegister(newUserMock)
	...
	test("makes a POST request to edit the data of an existing user", async () => {
    const storeUser = useStoreUserInfo();
    ...
    axios.post.mockResolvedValue({
      userData: editedUserMock,
    });

    const newUser = await storeUser.addUserOnRegister(editedUserData);

    const editedUserSpy = vi.spyOn(storeUser, "editUser");

    storeUser.editUser(editedUserMock);

    expect(editedUserSpy).toHaveBeenCalledWith(editedUserMock);
  });

	test("makes a GET request to retrieve a users data without losing id", async () => {
    const storeUser = useStoreUserInfo();

		var editedUserData = user
