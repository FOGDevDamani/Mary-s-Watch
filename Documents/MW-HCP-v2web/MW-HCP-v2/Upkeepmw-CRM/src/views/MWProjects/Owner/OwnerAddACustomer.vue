		...
		<form>
			<div class="row mt-4">
				<div class="col-lg-5 col-sm-12 mr-3">
					<MWFormLabelAndFileInput v-model="ownerCustomerData.profileImage" label="Upload Customer Profile Image" />					
				</div>
			</div>
	...
	*/
	const addOwnerCustomer = () => {
		if(ownerCustomerData) {
			const ownerCustomer = {
				data: state,
				image: ownerCustomerData.profileImage,
			}
			storeOwnerCustomer.addOwnerCustomer(ownerCustomer, user.value.uid)
		}
	}
