<template>
	<div>
		...
		<div class="row mt-5 pt-2">
			...
			<div class="col-sm-4">
				<AddCircleButton route="/owner-add-a-customer"></AddCircleButton>
			</div>
		</div>

		<div class="ms-5 ps-1 mt-5 pt-2 row row-cols-sm-3 g-4">
			<div v-for="customer in customerList" :key="customer.id" class="col">
				<MWCustomerCard  :profileImage="customer.data.profileImage" :firstName="customer.data.firstName" :lastName="customer.data.lastName" :address="customer.data.address" :email="customer.data.email" :cellPhone="customer.data.cellPhone" route="OwnerCustomerProfile" :id="customer.id"></MWCustomerCard>
			</div>
...
<script setup>
	...
	const { customers: customerList } =  storeToRefs(storeOwnerCustomer)

	onMounted(() => {
		const currentUID = user.value.uid
		storeOwnerCustomer.getCustomers(currentUID)
	})
