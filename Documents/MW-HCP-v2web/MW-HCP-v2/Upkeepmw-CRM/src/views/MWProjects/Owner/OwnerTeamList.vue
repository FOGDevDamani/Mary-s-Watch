<template>
	<div>
		...
		<div class="row mt-5 pt-2">
			...
			<div class="col-sm-4">
				<AddCircleButton route="/owner-add-a-team"></AddCircleButton>
			</div>
		</div>
		...
		<div class="ms-5 ps-1 mt-5 pt-2 row row-cols-sm-3 g-4">
			<div v-for="team in teamList" :key="team.id" class="col">
				<MWTeamCard  :profileImage="team.data.teamProfileImage" :label1="team.data.teamName" :label2="team.data.address" :label3="team.data.teamLead" route="OwnerTeamProfile" :id="team.id"></MWTeamCard>
			</div>
...
import { storeToRefs } from 'pinia'
	...
	*/
	...
	const { teams: teamList } =  storeToRefs(storeOwnerTeam)


	onMounted(() => {
		const currentUID = user.value.uid
		storeOwnerTeam.getTeams(currentUID)
	})
