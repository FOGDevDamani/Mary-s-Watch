		...
		<form>
			<div class="row mt-4">
				<div class="col-lg-5 col-sm-12 mr-3">
					<MWFormLabelAndFileInput v-model="spCustomerData.profileImage" label="Upload Customer Profile Image" />					
				</div>
			</div>
			...
			<button type="button" @click.prevent="addSPCustomer" class="btn btn-primary my-5 button">Add Customer</button>
		</form>
	</div>
</template>
...
<script setup>
	...
	const state = useStorage('spCustomerState', spCustomerData)

	const currentState = localStorage.getItem('spCustomerState')
	...
	*/
	const addSPCustomer = () => {
		if(spCustomerData) {
			const spCustomer = {
				data: state,
				image: spCustomerData.profileImage,
			}
			storeSPCustomer.addSPCustomer(spCustomer, user.value.uid)
		}
	}
